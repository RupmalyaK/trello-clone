...
`;
const AddCard = ({handleClose,handleSubmit,category}) => {
    const [showOptions,setShowOptions] = useState(false);
    const [colorIndex,setColorIndex] = useState(-1);
    const [title,setTitle] = useState("");
    const showColors = () => {
        const ColorComponenets = colorArr.map((color,index) => (
            <Color backgroundColor={color} onClick={e => {
                setColorIndex(index);
    ...
    return (<Container>
        ...
        <div className="buttons-container">
                <div className="left-side">
                    <button className="add-card-button-button" onClick={e => handleSubmit(title,colorIndex,category)}>Add Card</button>
      ...
      {showOptions &&  <div className="side-content">
            <div className="heading"style={{textAlign:"center",fontSize:"1.2rem",marginBottom:"5px"}}>Color</div>
